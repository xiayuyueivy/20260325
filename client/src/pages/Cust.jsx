...
export default function Cust() {
  ...
  const save = async () => {
    setError('');
    if (!form.cust_code.trim() || !form.cust_name.trim()) { setError('代碼與名稱為必填'); return; }
    try {
      if (isEdit) await api.put(`/cust/${form.cust_code}`, form);
      else         await api.post('/cust', form);
      setModal(false);
      load();
    } catch (e) {
  ...
  return (
    <div className="container">
      ...
      <div className="table-wrap">
        <table>
          <thead>
            <tr><th>客戶代碼</th><th>客戶名稱</th><th>備註</th><th>操作</th></tr>
          </thead>
          <tbody>
            {records.length === 0
              ...
              : records.map(r => (
                <tr key={r.cust_code}>
                  ...
                  <td className="action-cell">
                    ...
                    <button className="del-btn"  onClick={() => del(r.cust_code)}>刪除</button>
                  </td>
                </tr>
              ))}
          </tbody>
      ...
      {modal && (
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setModal(false)}>
          <div className="modal-card">
            <div className="modal-title">{isEdit ? '修改客戶' : '新增客戶'}</div>
            {error && <div className="alert alert-error">{error}</div>}
            <div className="form-group">
              <label>客戶代碼</label>
              <input value={form.cust_code} readOnly={isEdit}
                onChange={e => setForm({ ...form, cust_code: e.target.value })} placeholder="客戶代碼" />
            </div>
            <div className="form-group">
              <label>客戶名稱</label>
              <input value={form.cust_name}
                onChange={e => setForm({ ...form, cust_name: e.target.value })} placeholder="客戶名稱" />
            </div>
            <div className="form-group">
              <label>備註說明</label>
